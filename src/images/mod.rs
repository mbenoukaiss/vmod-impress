...
pub fn read<T>(path: T) -> Result<DynamicImage, Error>
where
    T: AsRef<Path>,
{
...
) -> Result<Box<dyn OptimizedImage>, Error> {
    let optimized: Box<dyn OptimizedImage> = match config {
        OptimizationConfig::Webp {
            quality,
            prefer_quality,
        } => Box::new(webp::to_webp(image, quality, prefer_quality)?),
        OptimizationConfig::Avif {
            quality,
            prefer_quality,
        } => Box::new(avif::to_avif(image, quality, prefer_quality)?),
        OptimizationConfig::Jpeg {
            quality,
            prefer_quality,
        } => Box::new(jpeg::to_jpeg(image, quality, prefer_quality)?),
    };

    Ok(optimized)
}

pub fn write<T>(path: T, data: &[u8], last_modified: Option<SystemTime>) -> Result<(), Error>
where
    T: AsRef<Path>,
{
    let directory = path
        .as_ref()
        .parent()
        .expect("Logic error: file should be in a directory");
